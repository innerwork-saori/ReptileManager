...
- 卡片式佈局，顯示照片縮圖、Morph 主標題（空值時回退為 Name / ID）、固定顯示名稱、種類、上次餵食時間、上次脫皮時間、最近體重、年齡、飼育箱位置
- 卡片式佈局採響應式排版：左側縮圖 + 右側資訊，桌機會放大縮圖並加寬內容區；Morph 主標題空值時回退為 Name / ID
- 每張卡片在縮圖右下提供浮動「編輯」快捷按鈕，桌機額外保留文字版「編輯」按鈕，可直接前往該爬蟲編輯頁
- 卡片資訊區塊採三層結構（標題資訊、三欄統計、快捷操作），統計欄固定顯示上次餵食、上次脫皮、最近體重
- 頂部搜尋欄：模糊搜尋名稱 / 種類 / 品系
- 分類篩選 Chip：全部 + 目前已有爬蟲資料使用中的分類（依 Category 欄位過濾）
- 從清單卡片進入「餵食」或「健康（脫皮 / 體重 / 就診）」並送出後，會返回清單並自動捲動回剛操作的該隻爬蟲卡片位置，且卡片會短暫高亮 1 秒提示定位結果
- 右下角 FAB 快速新增爬蟲
...
- 點擊主照片可開啟全螢幕預覽，預覽圖採置頂顯示，避免在長頁面中段錯位
...
- iOS 穩定性：主畫面 PWA 掃描流程加入相機啟動 fallback 與生命週期保護，降低授權後黑畫面機率
- iOS 啟動策略：改為兩階段啟動（先暖機請求相機權限，再延遲 500ms 啟動主掃描串流）
- iOS 自動恢復：偵測到 standalone 啟動異常時會延遲自動重試 1 次（避免無限重試）
- Fallback：提供手動輸入載荷 / URL / ID

### 3.12 歡迎頁（Welcome）

第一次使用時顯示的 App 介紹頁，說明理念與核心功能。

- 顯示條件：尚未看過歡迎頁（`settings` 的 `welcomeSeen`）且資料庫沒有任何爬蟲；已有資料的既有使用者不會看到
- 示範模式：每個示範工作階段一開始一定會顯示（重新產生範例資料時會清除 `welcomeSeen`）
- 僅在從首頁（`#/`）進入時導向，QR Code 等深層連結不受影響
- 內容：標語、理念說明、飼育箱標籤範例（照片、QR Code、上次餵食、體重曲線動畫）、三項核心功能、資料隱私說明
- 語言切換：頁首可直接切換中文 / English；示範模式下若切換語言，進入時會以新語言重新產生範例資料
- 操作：一般模式提供「新增第一隻爬寵」、「先看看範例資料」（開啟 `?demo=1`）、「略過介紹」；示範模式提供「開始看範例」與「開始使用我的資料」
- 動畫：僅體重曲線於載入時繪製一次，並遵循 `prefers-reduced-motion`

### 3.11 示範模式（Demo Mode）

提供給評審、展示或新使用者試用的示範環境，一般網址不會出現此功能。

- 入口：網址加上 `?demo=1`（例如 `https://<網域>/?demo=1`，位於 `#` 之前）
- 資料隔離：示範模式使用獨立的 IndexedDB `reptileManagerDb-demo`，不會讀寫使用者的正式資料庫 `reptileManagerDb`
- 範例資料：5 隻爬寵（球蟒 ×2、豹紋守宮 ×2、高冠變色龍），含餵食、體重曲線、脫皮、就診、投藥療程、環境、UVB、底材與 2 筆繁殖紀錄；日期皆相對於當下產生，並刻意讓 2 隻超過 10 天未餵食以展示首頁提醒
- 範例資料依當下介面語言（繁中 / 英文）產生名稱與食物類型
- 重新產生時機：每個瀏覽器工作階段（sessionStorage）首次開啟時重新產生；同一次瀏覽中的新增 / 修改在重新整理後仍保留
- 頂部提示條：顯示示範模式說明，提供「重置範例」（重新產生範例資料）與「開始使用我的資料」（移除 `?demo=1` 回到正式資料庫）
- 範例照片存放於 `public/demo/`（小芒果、奶油兩隻球蟒的實拍照，已縮圖以降低 PWA 預先快取大小；原圖保存於 `images/demo-originals/`）
...
| 份量 | 下拉選擇 1 至 5；選擇「更多」後可輸入自訂數字；可在餵食紀錄中修改數量 |
| 餵食時間 | datetime-local |
| 備注 | 自由文字 |
...
- 從「脫皮」相關入口（如爬蟲清單快捷按鈕、首頁脫皮資訊卡）進入健康頁時，預設會直接開啟「脫皮」分頁，其他入口維持預設「體重」分頁。
...
首頁採 Modern Nature 配色，主色遵循 Primary/Secondary/Tertiary/Neutral，並以語意 token 管理錯誤與警示狀態色：
- **太久沒吃 / 體重 / 脫皮**：餵食卡片列出超過 10 天未餵食的爬蟲（最多 5 隻），點擊可跳轉至對應爬蟲餵食頁
...
| 匯出 QR Code | 批次產生所有爬蟲 QR Code，每張顯示名稱與性別（M ♂ / F ♀ / Unknown），支援列印 / PDF |
| 匯入備份 | 上傳 JSON 檔，驗證後寫入 IndexedDB（覆蓋模式） |
| 聯絡支援 | 設定頁與側邊抽屜提供官方聯絡信箱 `reptilemanager@heysaori.com`，支援問題回報與功能建議 |
...
資料庫以 Dexie 定義，共 **15 張資料表**，全部儲存於瀏覽器 IndexedDB。示範模式（`?demo=1`）使用相同結構的獨立資料庫 `reptileManagerDb-demo`。
...
採 Hash 路由（`#/path`），確保 Netlify 靜態部署相容性。任何路由前加上 `?demo=1`（如 `/?demo=1#/breeding`）即進入示範模式，見 3.11。

| 路由 | 頁面 | 功能 |
|------|------|------|
| `/welcome` | WelcomePage | 第一次使用的 App 介紹頁（見 3.12） |
